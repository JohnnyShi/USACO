            ...
            for end_row in range(end_max_row, end_min_row - 1, -1):
                for end_col in range(end_max_col, end_min_col - 1, -1):
                    if is_pcl(start_row, start_col, end_row, end_col):
                        ...
                        count += 1  # this PCL
                        # skip bottom row, fix top-left corner and count the rest of PCL
                        count += search(start_row, start_row, start_col, start_col,
                                        start_row, end_row - 1, end_col + 1, end_max_col)

                        # shift top-left corner horizontally finish the rest of the top row
                        count += search(start_row, start_row, start_col + 1, start_max_col,
                                        start_row, end_max_row, start_col + 1, end_max_col)

                        # switch to new line, start from the left of start_col
                        next_line_count = search(start_row + 1, start_max_row, start_min_col, start_col - 1,
                                        start_row + 1, start_max_row, end_min_col, end_max_col)
                        if next_line_count > 0:
                            count += next_line_count
                            return count

                        # when start point is within PCL, the end point cannot be inside PCL
                        next_line_count = search(start_row + 1, end_row, start_col, end_col,
                                                 end_row + 1, end_max_row, start_col, end_max_col)
                        next_line_count += search(start_row + 1, end_row, start_col, end_col,
                        ...
                        # start point not inside PCL anymore, first count on the right
                        next_line_count = search(start_row + 1, end_row - 1, end_col + 1, end_max_col,
                                                 start_row + 1, end_max_row, end_col + 1, end_max_col)
                        # then count on the bottom
                        next_line_count += search(end_row + 1, end_max_row, start_col, end_col - 1,
                                                  end_row + 1, end_max_row, start_col, end_max_col)
                        ...
                        count += search(end_row, end_max_row, end_col, end_max_col,
                                        end_row, end_max_row, end_col, end_max_col)
                        return count
    return count
