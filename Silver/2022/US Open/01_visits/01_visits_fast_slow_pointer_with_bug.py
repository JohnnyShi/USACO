...
def find_cycle_min(start):
    ...
    # BUG: with settings on line 21 and 22, it's possible to have the slow move to a visited field when exit the loop
    # So should always set slow = a[start], fast = a[a[start]]
    # Cycle found
    min_value = v[slow]
    visited[slow] = True
    fast = a[slow]
